...
- [ ] **T1** — `:sdui-compose`: `ImageRegistry.kt` (espejo de `IconRegistry`): `ImageRegistry`
...
- [ ] **T2** — `:sdui-compose`: extender `ImageProps` con `name` y ramificar `ImageRenderer`
...
- [ ] **T3** — `:shared`: setup Compose Resources — dep `compose.components.resources` en `commonMain`;
...
- [ ] **T4** — `:shared`: `image/AppImages.kt` (`appImageRegistry()` = `imageRegistry { register(
...
- [ ] **T5** — `:server`: pantalla piloto. Añadir un `image(name="kuisd_logo")` (con `contentScale` y un
...
- [ ] `ImageRegistry` + `LocalImageRegistry` en `:sdui-compose` (espejo de `IconRegistry`), SIN assets propios.
- [ ] `image` resuelve `name`→local / `url`→remoto; retrocompat 012 intacta; fallback sin crash.
- [ ] `ImageRegistryTest` verde; `rendererFor("image") != null`.
- [ ] Asset `kuisd_logo` empaquetado en Compose Resources de `:shared` y registrado; `SduiHost` lo provee.
- [ ] `:sdui-compose:jvmTest`, `:shared:desktopTest`, `:server:build`, `detekt`, `ktlintCheck` en verde.
- [ ] Pantalla piloto sirve `image(name="kuisd_logo")` (T5) — evidencia e2e.
