...
- [x] **T1** — Contrato: `data object NavigateBack : UiAction` (`@SerialName("navigateBack")`) en
...
- [x] **T2** — MOTOR: `dev.kuisd.sdui/SduiActionHandler.kt` (`fun interface SduiActionHandler` +
  `LocalSduiActionHandler` default no-op).
  - _ref:_ HU-1.2 · design §MOTOR·SduiActionHandler
  - _verif:_ `:shared:assemble`.

- [x] **T3** — MOTOR: `RenderNode.kt` delega `node.actions["onClick"].orEmpty()` al
...
- [x] **T4** — DATA: crear `dev.kuisd.app.data` y **mover** `SduiClient.kt` y `SduiHttp.kt`(+actuals)
...
- [x] **T5** — APP: `dev.kuisd.app/ScreenUiState.kt` (sealed) + `SduiScreen.kt` (recibe `ScreenSource`,
...
- [x] **T6** — APP/nav: `dev.kuisd.app.nav/NavBackStack.kt` (`NavEntry` con `id`, snapshot-state, push/pop
...
- [x] **T7** — APP: `dev.kuisd.app/SduiHost.kt` (`@Composable`): posee `NavBackStack` + `KtorScreenSource`
...
- [x] **T8** — APP: `presentation/PlaceholderApp.kt` monta `SduiHost("home")`.
  - _ref:_ HU-4 · design §Mapa de paquetes
  - _verif:_ `:androidApp:assembleDebug` y `:desktopApp` compilan.

- [x] **T9** — SERVER: `screens/DetailsScreen.kt` ("Ver más"→`Navigate("more")`, "Atrás"→`NavigateBack`)
...
- [x] **T10** — `public`/`internal` según el mapa de diseño; comprobar regla de dependencias.
...
- [ ] **T11** — Smoke end-to-end de navegación: `:server:run` + `:desktopApp:run`; "Empezar"→`details`,
  ...
  - _Pendiente:_ requiere ventana desktop; no verificable de forma automatizada. El data-path quedó
    validado por smoke con `curl` (`/screen/home|details|more` → 200) y por los tests.
...
- [x] Motor `dev.kuisd.sdui` puro: solo render + delega vía `SduiActionHandler`; **no importa Ktor ni `dev.kuisd.app`** (grep CLEAN).
- [x] App `dev.kuisd.app` posee fetch (tras `ScreenSource`), estado y navegación (`NavBackStack`/`NavActionHandler`/`SduiHost`).
- [ ] Cadena `home→details→more` con "‹ Atrás" (y `NavigateBack` del server) funcional; raíz sin barra. _(smoke visual T11, manual)_
- [x] `NavigateBack` en el contrato, forward-compatible (→`NoOpAction`).
- [x] Acción no soportada / `onClick` vacío / ruta inexistente no crashean (cubierto por tests + `UnknownNode`).
- [x] API pública mínima (public/internal aplicado).
- [x] Todos los `:check`/`build`/`assembleDebug`/`detekt`/`ktlintCheck` en verde; `tasks.md` todo `[x]` salvo T11 (manual).
